...
function comparaPass(password, hash) {
    const compara = new Promise((resolve, reject) => {
        bcrypt.compare(password, hash, (err, res) => {
            if (err) reject({ status: 500, message: err });
            if (!res) reject({ status: 401, message: "contraseña Incorrecta" });
            else resolve({ status: 200, message: "contraseña Correcta" });
        });
    });
    return compara;
}

function comparaPassSync(password, hash) {
    return bcrypt.compareSync(password, hash)
}


module.exports = { createToken, decodeToken, codifica, comparaPassSync };